...
Create a DNS `A` record named `cleanup-tool` pointing to the server's public IPv4 address. Then open `http://cleanup-tool.myles-mattlock.co.uk`. Make sure TCP port 80 is allowed in the server firewall and cloud provider security group.
...
sudo certbot --nginx --redirect -d cleanup-tool.myles-mattlock.co.uk
```

The server should have a stable public IP before requesting the certificate.
